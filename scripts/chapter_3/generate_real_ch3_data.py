...
def generate_state_infrastructure(raw_dir, output_dir):
    ...
    def infer_state(row):
        ...
        if 'nsw' in city_addr or 'sydney' in city_addr:
            return 'NSW'
        if 'vic' in city_addr or 'melbourne' in city_addr:
            return 'VIC'
        if 'qld' in city_addr or 'brisbane' in city_addr:
            return 'QLD'
        if 'wa' in city_addr or 'perth' in city_addr:
            return 'WA'
        if 'sa' in city_addr or 'adelaide' in city_addr:
            return 'SA'
        if 'act' in city_addr or 'canberra' in city_addr:
            return 'ACT'
        return 'Unknown'
