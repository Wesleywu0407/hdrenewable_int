...
Run: python -m scripts.chapter_3.generate_ch3_charts
"""
...
def fig5_ai_dc_projections() -> None:
    ...
    deficit_y_top = [max(dc, re) for dc, re in zip(ai_dc_load_gw, planned_abs, strict=False)]
    deficit_y_bot = [min(dc, re) for dc, re in zip(ai_dc_load_gw, planned_abs, strict=False)]
    in_deficit = [dc > re for dc, re in zip(ai_dc_load_gw, planned_abs, strict=False)]
    shade_top = [t if d else b for t, b, d in zip(deficit_y_top, deficit_y_bot, in_deficit, strict=False)]
    shade_bot = [b if d else t for t, b, d in zip(deficit_y_top, deficit_y_bot, in_deficit, strict=False)]
