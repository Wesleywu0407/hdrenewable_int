...
Run: python -m scripts.chapter_1.fetch_qld_data
"""
...
from scripts.common.api import DEFAULT_FETCH_EXCEPTIONS, cache_is_fresh, configure_logging, summarize
...
def fetch_spot_prices(client: OEClient, region: str, days: int = 365) -> pd.DataFrame:
    ...
    while cur < now:
        ...
        except DEFAULT_FETCH_EXCEPTIONS as exc:
            LOGGER.warning("    price chunk %s failed: %s: %s", f"{cur:%Y-%m-%d}", type(exc).__name__, exc)
        cur = chunk_end
...
def save_and_merge(path: Path, df: pd.DataFrame) -> pd.DataFrame:
    if df is None or df.empty:
        return df
    if path.exists():
        LOGGER.info("  merging new data with existing historical data for %s...", path.name)
...
def main() -> int:
    ...
    with OEClient() as client:
        ...
        if cache_is_fresh(qld_path):
            ...
            LOGGER.info("[qld_facilities] cache hit (%s units).", len(qld))
        else:
            LOGGER.info("[qld_facilities] fetching QLD1 ...")
            qld = facilities_to_df(client, "QLD1")
            qld = save_and_merge(qld_path, qld)
            summarize(qld_path, qld, logger=LOGGER)
        cod_cov = qld["commissioning_date"].notna().mean() * 100
        LOGGER.info("  QLD units: %s | commissioning-date coverage: %.0f%%", len(qld), cod_cov)
        LOGGER.info("  status: %s", qld["status_id"].value_counts().to_dict())

        # --- Peer facilities (NSW, VIC) for Fig 4 -------------------------- #
        peers = {"QLD1": qld}
        for region in ["NSW1", "VIC1"]:
            LOGGER.info("[peer_facilities] fetching %s ...", region)
            peers[region] = facilities_to_df(client, region)
            LOGGER.info("  %s: %s units", region, len(peers[region]))
        ...
        summarize(ch_path, cap_hist, f"({cap_hist['month'].min():%Y-%m}..{cap_hist['month'].max():%Y-%m})", logger=LOGGER)
        ...
        summarize(pa_path, peer_add, logger=LOGGER)

        # --- QLD 24-month fuel mix (Fig 2) --------------------------------- #
        fm_path = RAW_DIR / "qld_fuel_mix_24m.csv"
        if cache_is_fresh(fm_path):
            LOGGER.info("[qld_fuel_mix_24m] cache hit.")
        else:
            LOGGER.info("[qld_fuel_mix_24m] fetching QLD1 monthly fuel mix ...")
            fm = fetch_qld_fuel_mix(client, months=24)
            fm = save_and_merge(fm_path, fm)
            summarize(fm_path, fm, f"({fm['interval'].min():%Y-%m}..{fm['interval'].max():%Y-%m})", logger=LOGGER)
        ...
        if cache_is_fresh(sp_path):
            ...
            LOGGER.info("[qld_spot_prices] cache hit (%s hours).", len(sp))
        else:
            LOGGER.info("[qld_spot_prices] fetching QLD1 hourly (chunked) ...")
            sp = fetch_spot_prices(client, "QLD1", days=365)
            if sp.empty:
                LOGGER.warning("  no spot price data returned - Fig 6 will be skipped.")
            else:
                sp = save_and_merge(sp_path, sp)
                summarize(sp_path, sp, f"({sp['interval'].min():%Y-%m-%d}..{sp['interval'].max():%Y-%m-%d})", logger=LOGGER)
                neg = (sp["price"] < 0).sum()
                LOGGER.info("  negative-price hours: %s (%.1f%% of %s)", neg, neg / len(sp) * 100, len(sp))

    LOGGER.info("")
    LOGGER.info("Done fetching QLD data.")
    return 0
